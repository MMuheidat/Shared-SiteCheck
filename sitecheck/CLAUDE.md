...
**Pillar 1 (Discovery) search flow:** engines are tried in the order Startpage → Yahoo → Bing → Google → DuckDuckGo, falling through only when an engine fails validation (captcha/blank/sign-in). Each engine is driven like a person: homepage → typed query (`pressSequentially`) → Enter, with the direct `?q=` URL as a same-engine fallback; DuckDuckGo's right-rail promos are hidden before evidence. Q1/Q1b tag the matching result anchor (`data-sitecheck-target`), highlight it, click it, and land on the entity site on video. Q1b uses `AuditJob.acronym` when set, else `deriveAcronym` (single-word names and embedded all-caps tokens like "TAMM" are preserved; otherwise word initials). Q3 types the entity name into Google Maps, opens the best-matching result, screenshots with the website field highlighted, and forces `target=_self` before clicking the website link (Maps opens sites in a new tab, which would not appear in the recorded page's video).
